...
Run `npm run samples` to regenerate `fixtures.json`, the inspectable HTML previews, and a disposable Vault under the ignored `samples/generated-vault/` directory. The fixture contains at least one catalog and analytics payload for every one of the 24 service packs. If Playwright and a Chrome channel are available locally, `bash scripts/capture-samples.sh` regenerates the 24 service screenshots.
...
These screenshots are design evidence. Runtime behavior is separately exercised by the renderer, template-upgrade, 10,000-entry performance, and 24-service atomic-sync tests.
